...
int enviar_mensaje(int id_cola_mensajes, long rLongDest, int rIntRte, int rIntEvento, char* rpCharMsg) {
    ...
    printf("Enviando mensaje: Destino=%ld, Remitente=%d, Evento=%d, Mensaje=%s\n", msg.dest, msg.rte, msg.evento, msg.mensaje);
...
int recibir_mensaje(int id_cola_mensajes, long rLongDest, mensaje* rMsg) {
    ...
    printf("Mensaje recibido: Destino=%ld, Remitente=%d, Evento=%d, Mensaje=%s\n", rMsg->dest, rMsg->rte, rMsg->evento, rMsg->mensaje);
    return res;
}
