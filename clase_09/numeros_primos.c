...
void *funcionPrimosThread(void *parametro) {
  ...
  *ptr = total; // utilizo *ptr para cargar el parametro de entrada el dato de
                // salida.
  pthread_exit((void *)0); // Esto solo puedo utilizarlo para devolver valo res
                           // de status, no puedo devolver valores creados en la
                           // propia funcion thread.
}
...
int main(int argc, char *argv[]) {
  ...
  char con_thread = 's';
  int i, total = 0;
  int cantidad = argc - 1; /* se va a recibir mas de un argumento */
