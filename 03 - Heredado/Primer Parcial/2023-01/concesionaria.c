...
int main(int argc, char *argv[])
{
 ...
 while (1)
 {
    ...
    if(memoria[vehiculoid].cantidad > 0)
	{
		 int cantidad = memoria[vehiculoid].cantidad;
		 if (cantidad > 4)
		 {
			 memoria[vehiculoid].cantidad = memoria[vehiculoid].cantidad - 4;
			 m[vehiculoid].cantidad = m[vehiculoid].cantidad+4;
		 }
		 else
		 {
			 m[vehiculoid].cantidad = m[vehiculoid].cantidad + memoria[vehiculoid].cantidad;
			 memoria[vehiculoid].cantidad = 0;
		 }
	}
    else
    {
		printf("No se se pudo comprar de ese vehiculo \n");
    }
